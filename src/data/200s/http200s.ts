...
export const http200s: IHttpStatusCode[] = [{
  ...
  meaning of "success" depends on the HTTP method:

  GET: "The resource has been fetched and transmitted
  in the message body."

  HEAD: "The representation
  headers are included in the response without any
  message body."

  PUT or POST: "The resource
  describing the result of the action is transmitted in
  the message body."

  TRACE: "The message body
  contains the request message as received by the server."`,
  ...
  led to the creation of a resource. The new resource
  is effectively created before this response is sent
  back and the new resource is returned in the body
  of the message, its location being either the URL
  of the request, or the content of the Location header.`,
  ...
  yet acted upon. It is noncommittal, since there is no
  way in HTTP to later send an asynchronous response
  indicating the outcome of the request. It is intended
  for cases where another process or server handles the
  ...
  metadata is not exactly the same as is available from
  the origin server, but is collected from a local or a
  third-party copy. This is mostly used for mirrors or
  backups of another resource. Except for that
...
},
{
  ...
  request, but the headers may be useful. The user
  agent may update its cached headers for this resource
  with the new ones.`,
  ...
  status tells the client to reset the document view, so
  for example to clear the content of a form, reset a
  ...
  Range header is sent from the client
  ...
  information about multiple resources in situations
  ...
  element to avoid repeatedly enumerating the internal
  ...
  resource, and the response is a representation of the result
  of one or more instance-manipulations applied to the
  current instance.`,
